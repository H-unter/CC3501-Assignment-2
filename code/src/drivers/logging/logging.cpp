...
void write_csv_header(SDCard &sd_card, FIL &file)
{
    ...
    snprintf(header_buffer, sizeof(header_buffer), "elapsed_time_ms,loadcell_voltage_v,loadcell_weight_kg,dac_voltage_v,leaf_temp_c\n");
...
{
    char row_buffer[128];
    snprintf(row_buffer, sizeof(row_buffer), "%llu,%.2f,%.2f,%.2f,%.2f\n",
             data.elapsed_time_ms, data.loadcell_voltage, data.loadcell_weight, data.dac_voltage, data.leaf_temperature);
