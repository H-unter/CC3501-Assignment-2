...
\code{uart_set_format()} function to configure the communication format correctly (data bits 
were set to 7, stop bits were set to 1, parity option was set to \code{UART_PARITY_EVEN}), 
allowing the RP2040 to receive the inverted data without additional circuitry. 

The \code{uart_set_break()} function was used assert a break condition on the UART transmission, where it was first set 
...
The following functions were then used to send an SDI-12 command to the sensors: \code{uart_write_blocking()}, \code{uart_tx_wait_blocking()}.
In order to read the sensor responses, the \code{uart_is_readable()} is used along with \code{tight_loop_contents()}, which 
